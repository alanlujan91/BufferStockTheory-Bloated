...
\subsection{If \GICAbs~Fails}

A consumer is `growth patient' if the perfect foresight growth
impatience condition fails (\cncl{\GICAbs}, $1 < \Pat/\PGro$).  Under
\cncl{\GICAbs} the constraint does not bind at the lowest feasible value of $\mRat_{t}=1$ because
...
\begin{equation}\begin{gathered}\begin{aligned}
  ...
  \\  \uFunc^{\prime}(1)  & < \Rfree \Discount \uFunc^{\prime}(\PGro)   \label{eq:EulerGICAbsFailsEnd}.
...
\hypertarget{cnclGICAbscnclRICFHWC}{}
...
\hypertarget{cnclGICAbscnclRICcnclFHWC}{}
...
implies that for a consumer satisfying \cncl{\GICAbs} we must impose
the \RIC~(and the \FHWC~can be shown to be a consequence of \cncl{\GICAbs} and \RIC).  In
...
\end{aligned}\end{gathered}\end{equation}
which (under these assumptions) satisfies $0 < \mRat_{\#} < 1$.\footnote{Note that $0 < \mRat_{\#}$ is implied by \RIC~and $ \mRat_{\#}<1$ is implied by \cncl{\GICAbs}.}  For
...
\subsection{If \GICAbs~Holds}

Imposition of the \GICAbs~reverses the inequality in
\eqref{\localorexternallabel{eq:EulerGICAbsFailsEnd}}, and thus
...
the \GICAbs, the constraint certainly binds in period $t$ (and
...
\end{eqnarray*}
Given the \GICAbs~$\PatPGro^{-1}>1$, this will hold iff the \RIC~holds, $\PatR < 1$.  But given that the \FHWC~$\Rfree > \PGro$ holds, the \GICAbs~is stronger (harder to satisfy) than the \RIC; thus, the~\FHWC~and the \GICAbs~together imply the \RIC, and so a well-defined
...
% Finally, the main text shows that the \GICAbs~is a stronger condition than the PF-FVR; that is, \GICAbs~$\Rightarrow$ PF-FVR.
...
\begin{comment}
  ...
  in strictly fewer circumstances) than the \GICAbs~condition $\PatPGro < 1$.
  Thus, the \GICAbs~is an implication of $\cncl{\FHWC}$.
\end{comment}
...
\end{aligned}\end{gathered}\end{equation}
\hypertarget{GICAbsHoldsFHWCFailsRICFailsDiscuss}{}
...
\begin{eqnarray*}
  ...
  \\ \PGro  & > & \Pat
\end{eqnarray*}
which is merely the \GICAbs~which we are maintaining.  So the first term's limit is $+\infty$.  The
...
We can summarize as follows.  Given that the \GICAbs~holds, the
