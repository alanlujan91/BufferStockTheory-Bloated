\input{./econtexRoot}\documentclass[\econtexRoot/BufferStockTeory.tex]{subfiles}
\begin{document}

\let\TableWidth\relax
\newlength\TableWidth
\providecommand{\FigsRawApndx}{\econtexRoot/Code/Mathematica/Results/BufferStockTheory/Figures}
%\providecommand{\FigsRawApndx}{\econtexRoot/Figures}
...
choose $\cRat = \mRat$ from equation \eqref{eq:cFuncPFUnc}:
\begin{align}
...
\eqref{eq:EulerGICRawFailsEnd}, and thus
...
\end{eqnarray*}
Given the \GICRaw~$\PatPGro^{-1}>1$, this will hold iff the \RIC~holds, $\PatR < 1$.  But given that the \FHWC~$\Rfree > \PGro$ holds, the \GICRaw~is stronger (harder to satisfy) than the \RIC; thus, \FHWC~and the \GICRaw~together imply the \RIC, and so a well-defined
...
\input{\econtexRoot/\ApndxDir/onlyinsubfile.sty/Figures/PFGICHoldsFHWCFailsRICFails.tex}
