...
$\usual{\cFunc}(\mRat)$ for integer values of $n$ (e.g.,
$\cRat_{\#}^{n} = \PatPGro^{-n}$) are continuous functions of $n$; the
conclusion that $\usual{\cFunc}(\mRat)$ is piecewise linear
...
points.\footnote{In practice, we calculate the first and second
  derivatives of $\usual{\cFunc}$ and use piecewise polynomial
