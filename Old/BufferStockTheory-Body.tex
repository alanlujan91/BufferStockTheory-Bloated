...
\begin{center}
\begin{tabbing}
\texttt{~Archive:~} \= \= \url{https://www.econ2.jhu.edu/people/ccarroll/BufferStockTheory.zip} \kill \\  %
\texttt{~~~~~PDF:~} \> \> \url{https://econ-ark.github.io/BufferStockTheory.pdf} \\
\texttt{~~Slides:~} \> \> \url{https://econ-ark.github.io/BufferStockTheory-Slides.pdf} \\
\texttt{~~~~~Web:~} \> \> \url{https://econ-ark.github.io/BufferStockTheory/}    \\
...
\thanks{All numerical results herein were produced using the \href{https://econ-ark/HARK}{Econ-ARK/HARK} toolkit, which can be cited per our references (\cite{carroll_et_al-proc-scipy-2018}); for further reference options see \href{https://econ-ark.org/acknowledging/}{Acknowleding Econ-ARK}.      Thanks to James Feigenbaum, Joseph Kaboski, Miles Kimball, Qingyin Ma, Misuzu Otsuka, Damiano Sandri, John Stachurski, Adam Szeidl, Metin Uyanik, Weifeng Wu,
...
In the presence of empirically realistic transitory and permanent shocks to income \textit{a la} \cite{friedmanATheory}, only one additional ingredient is required to define a testable model of optimal consumption: A utility specification.  Modelers usually assume geometric discounting of a constant relative risk aversion (CRRA) utility function, because, starting with Zeldes~\citeyearpar{zeldesStochastic}, a large literature has shown that such models have quantitative predictions that can match microeconomic evidence reasonably well.
...
This paper's first technical contribution is to articulate the (surprisingly loose) conditions under which the simple problem (without convenient shortcuts like a consumption floor or liquidity constraints) defines a contraction mapping with a nondegenerate consumption function (the main requirement is a \hyperlink{FVAC}{`Finite Value of Autarky'} condition).  Another contribution is to specify the conditions under which the resulting consumption function implies there is a `target' wealth-to-permanent-income ratio.  (This is the sense in which the paper studies the class of `buffer stock' saving models.)  The key requirement is that the model's parameters satisfy a \hyperlink{GICNrm}{``Growth Impatience Condition''} (equation \eqref{eq:GICNrm}) that relates preferences and uncertainty to the predictable growth rate of income.

Even without a formal proof, target saving of this kind has been understood to underlie central results from the heterogeneous agent macroeconomics literature; for example, the logic of target saving is central to the explanation by \cite{kmpHandbook} of the fact that, during the Great Recession, middle-class consumers cut their consumption more than the poor or the rich. The theoretical foundations provided here are valuable both because they provide intuition about the determinants of saving targets, and because they make it easier to develop reliable numerical solution methods (by providing new restrictions that valid solutions must satisfy).

Conveniently, elements required for the convergence proof turn out to provide analytical foundations for other results that have become familiar from the numerical literature.  All theoretical conclusions are paired with numerically computed illustrations (using an open-source toolkit available from the \href{https://github.com/econ-ark/REMARK/blob/master/REMARKs/BufferStockTheory/BufferStockTheory.ipynb}{Econ-ARK} project).  All of the insights of this paper are instantiated in the toolkit, which algorithmically flags parametric choices under which a problem fails to define a contraction mapping, under which a target level of wealth does not exist, or under which the solution is otherwise degenerate.
...
\usual{\MPCFunc}(\mRat) = 0$, nevertheless the limiting constrained
consumption function $\usual{\cFunc}(\mRat)$ is strictly positive
...
$\usual{\cFunc}(\mRat)=0$ while \cncl{\FHWC} would suggest a
degenerate limit of $\usual{\cFunc}(\mRat)=\infty$.
...
\hypertarget{GICNrm}{}
We can now transparently generalize the \GICRaw~\eqref{eq:GICRaw} by defining a `compensated growth factor'
...
\end{align*}
which implies that if we wish to prevent $\mRat$ from heading to infinity (that is, if we want $m$ to be guaranteed to be expected to fall for some large enough value of $m$) we must impose a generalized version of the Perfect Foresight Growth Impatience Conditon \eqref{eq:GICRaw} which we call simply the `growth impatience condition' 
(\GICNrm):\footnote{Equation \eqref{eq:GICNrm} is a bit easier to satisfy than the similar condition imposed by Deaton~\citeyearpar{deatonLiqConstr}: $\left(\Ex[\pShk^{-\CRRA}]\right)^{1/\CRRA} \PatPGro < 1$ to guarantee that his problem defined a contraction mapping.}
\begin{verbatimwrite}{\EqDir/GICNrm}
\begin{align}
  \PatPGroAdj  & < 1 \label{eq:GICNrm}
\end{align}\end{verbatimwrite}
\input{\EqDir/GICNrm}
which is stronger than the perfect foresight version \eqref{eq:GICRaw} because $\PGroAdj < \PGro$.
...
\begin{comment}
Equation
  \eqref{eq:GICNrm} can be raised to the $\CRRA$ power yielding the
...
\input{\econtexRoot/\TableDir/Parameters.tex}
...
\{\GICRaw,\cncl{RIC}\} case (for detailed analysis of this
...
\subsubsection{When the \GICNrm~Fails}

If both the \GICNrm~and the \RIC~hold, the arguments above establish that the limiting consumption
...
ratio $\mTrg$ such that if $\mRat_t = \mTrg$ then $\Ex_t [
...
\mTrg$ then cash-on-hand will rise (in expectation), while if
...
Define the target cash-on-hand-to-income ratio $\mTrg$ as the value of $\mRat$
such that
\begin{equation}  
\Ex_t [{\mRat}_{t+1}/\mRat_t] = 1 \mbox{~if~} \mRat_t = \mTrg. \label{eq:mTarget}
\end{equation}
...
\\  & < 1
\end{align*}
where the last line is guaranteed by our imposition of the \GICNrm~\eqref{eq:GICNrm}.

Stability means that in a local neighborhood of $\mTrg$, values of $\mRat_{t}$
above $\mTrg$ will result in a smaller ratio of $\Ex_{t}[{\mRat}%
_{t+1}/\mRat_{t}] $ than at $\mTrg$. That is, if $\mRat_{t} > \mTrg$ then $%
\Ex_{t}[\mRat_{t+1}/\mRat_{t}] < 1 $. This will be true if
\begin{align}
\left(\frac{d}{d\mRat_t}\right) \Ex_t [{\mRat}_{t+1}/\mRat_t ]  & < 0 \label{eq:stable} \nonumber
\end{align}
at $\mRat_{t}=\mTrg$.  But \providecommand{\numFunc}{\pmb{\zeta}}
\begin{align*}
...
The target level of market resources is the $\mRat$ such that if $\mRat_{t}=\mTrg$ then $\Ex_{t}[\mRat_{t+1}]=\mTrg$.
\begin{align}
\Ex_{t}[{\mRat}_{t+1}]  & = \Ex_{t}[{\Rnorm}_{t+1}(\mRat_{t}-c_{t})+{%
\tShkAll}_{t+1}] \nonumber \\
\mTrg  & = \bar{\Rnorm}(\mTrg-\cFunc(\mTrg))+1  \nonumber \\
\bar{\Rnorm}\cFunc(\mTrg)  & = 1+(\bar{\Rnorm}-1)\mTrg  \label{eq:cstar}.
\end{align}
...
\begin{align*}
\numFunc(\mTrg)  & = 1+(\bar{\Rnorm}-1)\mTrg- \bar{\Rnorm}\cFunc^{\prime}(\mTrg)\mTrg - 1 \\
 & = \mTrg\left(\bar{\Rnorm}-1-\bar{\Rnorm}\cFunc^{\prime}(\mTrg)\right) \\
 & = \mTrg\left(\bar{\Rnorm}(1-\cFunc^{\prime}(\mTrg))-1\right) \\
 & < \mTrg\left(\bar{\Rnorm}(1-(1-{\Rfree}^{-1}(\Rfree\DiscFac)^{1/%
\CRRA}))-1\right) \\
 & = \mTrg\left(\bar{\Rnorm}\PatR-1%
\right) \\
 & = \mTrg\left(\underbrace{\Ex_{t}[{\Pat}/{\PGro}_{t+1}]}_{<
1\text{~from \eqref{eq:GICNrm}}}-1\right) \\
 & < 0
\end{align*}
...
We have now proven that some target $\mTrg$ must exist, and that at any such
$\mTrg$ the solution is stable. Nothing so far, however, rules
...
e.g., $\mTrg^{1}$. The argument just completed implies that since
...
\mTrg^{1}+\epsilon$. (Continuous differentiability of
...
definition of a target, $\mTrg^{2}$. Since
$\Ex_{t}[{\mRat}_{t+1}/\mRat_{t}]$ is continuous, it must be
approaching 1 from below as $\mRat_{t} \rightarrow \mTrg^{2}$, since
by the intermediate value theorem it could not have gone above 1
between $\mTrg^{1}+\epsilon$ and $\mTrg^{2}$ without passing through
1, and by the definition of $\mTrg^{2}$ it cannot have passed through
1 before reaching $\mTrg^{2}$.  But saying that
$\Ex_{t}[{\mRat}_{t+1}/\mRat_{t}]$ is approaching 1 from below as
$\mRat_{t} \rightarrow \mTrg^{2}$ implies that
\begin{align}
\left(\frac{d}{d\mRat_{t}}\right)\Ex_{t}[{\mRat}_{t+1}/\mRat_{t}]  & > 0
\label{eq:badeqn}
\end{align}
at $\mRat_{t}=\mTrg^{2}$. However, we just showed above that, under
our assumption that the \GICNrm~holds, precisely the opposite of equation
...
restrained model will be $\mTrg = \Ex_{t}[{\tShkAll}_{t+1}] = 1$. That
