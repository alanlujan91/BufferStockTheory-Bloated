...
# %% [markdown]
# ## If the GIC Holds, $\exists$ a "Balanced Growth" $m$
#
# $\hat{\hat{m}}$ is defined as a value of $m$ at which the consumer expects `balanced growth` in their market resources and their permanent income:
#
# \begin{align*}
...
# So $\hat{m}$ will be
# \begin{align*}
# \hat{\hat{m}} & =(\Rfree/\Gamma) (\hat{\hat{m}}-c(\hat{\hat{m}}))+1
# \\ \hat{\hat{m}}-1 & =(\Rfree/\Gamma) (\hat{\hat{m}}-c(\hat{\hat{m}}))
# \\ (\Gamma/\Rfree)(\hat{m}-1) & =\hat{\hat{m}}-c(\hat{\hat{m}})
# \\ c(\hat{\hat{m}}) & = \hat{\hat{m}}(1-(\Gamma/\Rfree)) +(\Gamma/\Rfree)
# \end{align*}
#
