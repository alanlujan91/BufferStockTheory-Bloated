...
\end{align}
where $\Rnorm_{t+1}\equiv (\Rfree/\PGro_{t+1})$ is a `growth-normalized' return factor, and the new problem's first order condition is\footnote{Leaving aside their assumptions about the utility function and liquidity constraints, it is tempting to view the model here as a special case of the model of MST, with $\Rnorm_{t+1}$ corresponding to their stochastic rate of return on capital and $\DiscFac \PGro_{t+1}^{1-\CRRA}$ to their stochastic discount factor.  But a potentially important caveat is that, here, $\Rnorm_{t+1}$ and the modified discount factor are intimately related because $\PGro_{t+1}$ plays a role in each.  Perhaps this is why relative risk aversion is present in the {\FVAC} condition but not in the corresponding condition in MST.}
\begin{align}
