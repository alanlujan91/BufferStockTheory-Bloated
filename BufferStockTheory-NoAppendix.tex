...
\begin{footnotesize}
  \parbox{\textwidth}{
    ...
    The \href{https://econ-ark.org/materials/BufferStockTheory?dashboard}{dashboard} will launch a live interactive \href{https://https://econ-ark.org/materials/BufferStockTheory?launch}{Jupyter Notebook} that uses the \href{https://econ-ark/HARK}{Econ-ARK/HARK} toolkit to produce all of the paper's figures (warning: the dashboard may take several minutes to launch).
  } % end parbox{\textwidth}
\end{footnotesize}
...
In the presence of empirically realistic transitory and permanent shocks to income \textit{a la} \cite{friedmanATheory},\footnote{As formalized by \cite{muthOptimal}.} only one more ingredient is required to construct a testable model of optimal consumption: A description of preferences.  Zeldes~\citeyearpar{zeldesStochastic} was the first to construct a quantitatively realistic version of such a model (with only preferences and shocks), spawning a subsequent literature that has shown that such models' predictions match microeconomic evidence reasonably well; results are similar whether or not liquidity constraints are imposed.\footnote{For life cycle models without constraints, see \cite{carrollBSLCPIH}; \cite{gpLifeCycle}; for a model with constraints that produces very similar results, see, e.g., \cite{Cagetti}; and for an infinite horizon model without constraints that resembles \cite{deatonLiqConstr}'s constrained model, see \cite{carrollBrookings}.}

A companion theoretical literature has derived key analytical properties of infinite-horizon solutions, but only for models more complex than the case with just shocks and preferences.  The extra complexity has been required, in part, because standard contraction mapping theorems (beginning with \cite{bellmanDynamicProgramming} and including those building on Stokey et.~al.~\citeyearpar{slpMethods}) cannot be applied when utility or marginal utility are potentially unbounded.  Many proof methods also rule out permanent shocks \textit{a la} \cite{friedmanATheory}, \cite{muthOptimal}, and \cite{zeldesStochastic}.  (See \hyperlink{DiffFromLit}{the fuller discussion} at the end of section \ref{subsec:Setup}).

This paper's first technical contribution is to articulate conditions under which the simple problem (without embellishments like a consumption floor or liquidity constraints) defines a contraction mapping with nondegenerate value and consumption functions.  The interesting requirement is a \hyperlink{FVAC}{`Finite Value of Autarky'} condition; conveniently, the resulting consumption function has analytical properties, like continuous differentiability, that make it easier to analyze than the embellished models.  The paper's second theoretical contribution is to specify the conditions under which a `target' wealth-to-permanent-income ratio exists (the consumer exhibits `buffer stock' saving behavior).  Buffer stock saving, variously defined, arises when the model's parameters satisfy some version of a \hyperlink{GIC}{``Growth Impatience Condition''} that relates preferences and uncertainty to predictable income growth.

\hypertarget{KMP}{}

Even without a formal proof, target saving has been intuitively understood to underlie central quantitative results from the heterogeneous agent macroeconomics literature; for example, the logic of target saving is central to the explanation in the \textit{Handbook of Macroeconomics} by \cite{kmpHandbook} of the fact that, during the Great Recession, middle-class consumers cut their consumption more than the poor or the rich.  The theory below explains why:  Learning that the future has become more uncertain does not change the urgent imperatives of the poor (their high $\uFunc^{\prime}(\cRat)$ means they -- optimally -- have little room to maneuver).  And, increased labor income uncertainty does not change the behavior of the rich because it poses little risk to their consumption.  Only people in the middle have both the motivation and the wiggle-room to reduce their discretionary spending when uncertainty increases.

Elements required for the proofs also provide foundations for many other results familiar from the numerical literature.  Analytical derivations of these results are provided along the way to the proofs, paired with numerically computed illustrations (made using the open-source \href{https://econ-ark.org}{Econ-ARK} toolkit).  The main insights of the paper are instantiated in the toolkit, whose \href{https://hark.readthedocs.io/en/stable/reference/ConsumptionSaving/ConsIndShockModel.html}{buffer stock saving module} flags parametric choices under which a problem may be degenerate or a target level of wealth may not exist.% (and also reports other implications derived here).
...
\end{align*}
  Note that the last version of the \PFFVAC~in \eqref{eq:PFFVAC} implies the \PFGIC~$\PatPGro < 1$ whenever \cncl{\FHWC} ($\Rfree < \PGro$) holds.  So, if \cncl{\FHWC}, value for any finite $\mRat$ will be the sum of two finite numbers: The component due to the unconstrained consumption choice made over the finite horizon leading up to $\bRat_{t+n} = 0$, and the finite component due to the value of consuming all $\pLevBF_{t+n}$ thereafter.
...
\{\PFGIC,\cncl{\RIC}\} case.  There, too, the tension between infinite human wealth
...
In the perfect foresight unconstrained model, we showed that the combination of the {\PFFVAC} and the {\FHWC}
is equivalent to the {\RIC}:
  \begin{align*}
  \overbrace{\DiscFac \PGro^{1-\CRRA} }^{\equiv \DiscAlt}  & < 1  \nonumber 
  \\ \DiscFac \Rfree \PGro^{-\CRRA}  & < \Rfree/\PGro \label{eq:PFFVAC} 
  \\ \Pat      & < \Rfree^{1/\CRRA}\PGro^{1-1/\CRRA}  \nonumber
  \\ \PatR     & < \Rfree^{1/\CRRA-1}\PGro^{1-1/\CRRA}  \nonumber
  \\ \PatR     & < (\PGro/\Rfree)^{1-1/\CRRA}  
  \end{align*}
since for $\CRRA > 1$ The exponent on the right hand side will be a positive number so if $\PGro/\Rfree < 1$ then $(\PGro/\Rfree)^{1-1/\CRRA}<1$.

The corresponding derivation for the case with uncertainty is
  \begin{align*}
  \overbrace{\DiscFac \PGrouAdj^{1-\CRRA} }^{\equiv \DiscAltuAdj}  & < 1  \nonumber 
  \\ \DiscFac \Rfree \PGrouAdj^{-\CRRA}  & < \Rfree/\PGrouAdj \nonumber
  \\ \Pat      & < \Rfree^{1/\CRRA}\PGrouAdj^{1-1/\CRRA}  \nonumber
  \\ \PatR     & < \Rfree^{1/\CRRA-1}\PGrouAdj^{1-1/\CRRA}  \nonumber
  \\ \PatR     & < (\PGrouAdj/\Rfree)^{1-1/\CRRA}  < (\PGro/\Rfree)^{1-1/\CRRA} < 1
  \end{align*}
  because $\PGrouAdj < \PGro$.  So if the {\FVAC} and {\FHWC} hold, the {\RIC} must hold.

  What can we say if the {\FVAC} fails but {\FHWC} holds?
  \begin{align*}
  \overbrace{\DiscFac \PGrouAdj^{1-\CRRA} }^{\equiv \DiscAltuAdj}  & > 1  \nonumber 
  \\ \DiscFac \Rfree \PGrouAdj^{-\CRRA}  & > \Rfree/\PGrouAdj \nonumber
  \\ \Pat      & > \Rfree^{1/\CRRA}\PGrouAdj^{1-1/\CRRA}  \nonumber
  \\ \PatR     & > \Rfree^{1/\CRRA-1}\PGrouAdj^{1-1/\CRRA}  \nonumber
  \\ \PatR     & > (\PGrouAdj/\Rfree)^{1-1/\CRRA}  
  \end{align*}
  but because $\PGrouAdj < \PGro$ it is possible for the following configuration to arise:
  \begin{align*}
  \overbrace{\DiscFac \PGrouAdj^{1-\CRRA} }^{\equiv \DiscAltuAdj}  & > 1  \nonumber 
  \\ \DiscFac \Rfree \PGrouAdj^{-\CRRA}  & > \Rfree/\PGrouAdj \nonumber
  \\ \Pat      & > \Rfree^{1/\CRRA}\PGrouAdj^{1-1/\CRRA}  \nonumber
  \\ \PatR     & > \Rfree^{1/\CRRA-1}\PGrouAdj^{1-1/\CRRA}  \nonumber
  \\ 1 > (\PGro/\Rfree)^{1-1/\CRRA} & > (\PGrouAdj/\Rfree)^{1-1/\CRRA}  = (\PGro/\Rfree)^{1-1/\CRRA}\uInvEpShkuInv^{1-1/\CRRA}
  \end{align*}

  We are interested in circs under which {\cncl{\FVAC}} and {\FHWC} are consistent with $\PatR < 1$
  \begin{align*}
    1     & > \PatR > (\PGrouAdj/\Rfree)^{1-1/\CRRA}  \nonumber
  \\ 1 & > (\PGro/\Rfree)^{1-1/\CRRA}  > (\PGrouAdj/\Rfree)^{1-1/\CRRA}  
  \end{align*}


However, if we define a growth-adjusted return factor  we can rewrite this as 
    \begin{align*}
  \overbrace{\DiscFac \PGrouAdj^{1-\CRRA} }^{\equiv \DiscAltuAdj}  & > 1  \nonumber 
  \\ \DiscFac \Rfree \PGrouAdj^{-\CRRA}  & > \Rfree/\PGrouAdj \nonumber
      \\ \Pat      & > \Rfree^{1/\CRRA}\PGrouAdj^{1-1/\CRRA} \equiv (\Rfree/\uInvEpShkuInv)^{1/\CRRA}\PGrouAdj\PGro^{-1/\CRRA}  \nonumber
  \\ \Pat      & > \Rfree^{1/\CRRA}\PGrouAdj^{1-1/\CRRA} \equiv (\Rfree/\uInvEpShkuInv)^{1/\CRRA}\PGrouAdj\PGro^{-1/\CRRA}  \nonumber                     
  \end{align*}


  \begin{align}
    \DiscFac \PGrouAdj^{1-\CRRA} & > 1 >     \DiscFac \PGro^{1-\CRRA}
\\   \PGro \uInvEpShkuInv & < \DiscFac^{1/(1-\CRRA)} <      \PGro
\\   \uInvEpShkuInv & < \DiscFac^{1/(1-\CRRA)}/\PGro <      1
  \end{align}

    \begin{align}
    \DiscFac \PGro^{1-\CRRA}\uInvEpShkuInv^{1-\CRRA} & > 1 >     \DiscFac \PGro^{1-\CRRA}
\\    \uInvEpShkuInv^{1-\CRRA} & > (\DiscFac^{-1}\PGro^{\CRRA-1}) >  1
\\    \uInvEpShkuInv^{\CRRA-1} & < \DiscFac\PGro^{1-\CRRA} <  1
\\    \uInvEpShkuInv & < (\DiscFac^{1/(\CRRA-1)}/\PGro) <  1
  \end{align}

The {\FVAC} is sufficient, but might not be necessary, for a solution.

Suppose it fails, which we will write in one of several potential ways:

\begin{align}
  \PatPGro & > (\Rfree/\PGro)^{1/\CRRA}\uInvEpShkuInv^{1-1/\CRRA}=(\Rfree/\PGrouAdj)^{1/\CRRA}\uInvEpShkuInv^{1}
\\ (\Rfree \DiscFac)^{1/\CRRA} & > (\Rfree/\PGro)^{1/\CRRA}\PGro\uInvEpShkuInv^{1-1/\CRRA}
\end{align}

Now consider the {\FHWC} case where $\Rfree/\PGro > 1$.  Since $\uInvEpShkuInv < 1$, failure of the \FVAC then requires 

In the special case of $\CRRA=2$, $\InvEpShkInv=\uInvEpShkuInv$ in which case the second line above becomes
\begin{align}
  \PatPGroAdj > (\Rfree/\PGroAdj)^{1/\CRRA}
\end{align}
in which case if the a growth-modified version of the FHWC holds, $(\Rfree/\PGroAdj) > 1$ then the GIC fails.  

The conditions of the theorem were sufficient, but we did not prove they were necessary, for the problem to define
a contraction mapping.

An interesting edge case might be to start at the $\PGroAdj$ node and reverse the arrow to $\Pat$ (imposing \cncl{\GIC}), then impose the {\RIC}:
\begin{align}
  \PGroAdj & < \Pat
  \\  \Pat & < \Rfree
\\ \PGroAdj & < \Pat < \Rfree             
...
Had we started at the $\PGro$ node we would not be able, from here, to impost \cncl{\FHWC} because that would lead to $\PGro < \PGro$.  But we have a little bit of wiggle-room from the fact that $\PGroAdj < \PGro$.  Imposing \cncl{\FHWC} would lead us to the proposition 
\begin{align}
\\ \PGroAdj & < \Pat < \Rfree < \PGro            
\\ \InvEpShkInv & < \Pat/\PGro < \Rfree/\PGro < \PGro/\PGro = 1
\end{align}

What this edge case tells us is that if we start by imposing \cncl{GIC}, we can guarantee that the {\FVAC} holds by imposing the combination {\RIC}, \cncl{\FHWC}, and the condition above.
...
\ref{fig:cGroTargetFig}), there is a target $\mTarg$ such that if $\mRat_t = \mTarg$ then $\Ex_t [
...
If constrained consumers are sufficiently impatient, the limiting target level model will be $\mTarg = \Ex_{t}[{\tShkAll}_{t+1}] = 1$. That
