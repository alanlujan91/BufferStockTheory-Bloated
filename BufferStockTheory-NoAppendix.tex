...
Analytical derivations required for the proofs also provide foundations for many other results familiar from the numerical literature.  The main insights of the paper are instantiated in the \href{https://econ-ark.org}{Econ-ARK} toolkit, whose \href{https://hark.readthedocs.io/en/stable/reference/ConsumptionSaving/ConsIndShockModel.html}{buffer stock saving module} flags parametric choices under which a problem is degenerate or a target level of wealth may not exist.% (and also reports other implications derived here).
...
The most peculiar possibility occurs when the \RIC~fails.  Under these circumstances the \FHWC~must also fail (Appendix~\ref{sec:ApndxLiqConstr}), and the constrained consumption function is nondegenerate.  (See appendix Figure~\ref{fig:GICAbsHoldsFHWCFailsRICFails} for a numerical example).  While it is true that $\lim_{m \uparrow \infty}
...
\begin{theorem}
 ...
 The full proof is in Appendix~\ref{}, but the key points can be made informally here.  Defining $\bar{\Rnorm}=(\Rfree/\PGroAdj)$, since $\Ex_{t}[\mRat_{t+1}]=\bar{\Rnorm}(\mRat_{t}-\cRat_{t})+1$, the implicit equation for $\mTarg$ is 
 \begin{align}
...
As an input to our more focused small-open-economy analysis (in section~\ref{}) it will be useful to derive here the conditions under which an $\mRat$ will exist at which an individual consumer expects `balanced growth' in their own individual market resources and permanent income:
  \begin{align}
...
The proofs of the two theorems are almost completely parallel; to save space, they are relegated to~\ref{sec:ApndxTargetMIsStable}.  In sum, they involve three steps:
