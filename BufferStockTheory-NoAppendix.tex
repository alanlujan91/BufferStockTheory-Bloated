...
The next theoretical contribution is to show that a corresponding model with an `artificial' liquidity constraint (a model that prohibits borrowing, even by consumers who could certainly repay) is a limiting case of the model without constraints.  The analytical appeal of the unconstrained model is that it is both mathematically convenient (e.g., the consumption function is twice continuously differentiable), and arbitraily close (cf.\ Section~\ref{sec:deatonIsLimit}) to less tractable models that have elsewhere been tackled with less convenient methods.  This treatment models a strategy of proving interesting propositions in a congenial environment, and then appealing to a limiting argument to define the analogous proposition as holding (in the limit) in a more unwieldy environment.
