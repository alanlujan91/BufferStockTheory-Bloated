...
\renewcommand{\forcedate}{October 8, 2021}\date{\forcedate}
...
We next sketch the perfect foresight constrained solution because it is a useful benchmark (and limit) for the unconstrained problem with uncertainty (examined next).
...
\end{align}
which is stronger than our {\GIC} condition $\PatPGro < 1$.\footnote{Under our default (though not required) assumption that $\log \pShk \sim \mathcal{N}(-\sigma^{2}_{\pShk}/2,\sigma^{2}_{\pShk})$; the {\GICNrm} in this case, is $\PatPGro < \exp(-\sigma^{2})$, so if the {\GICNrm} holds then Szeidl's condition will hold.}  %Below we designate these the `Szeidl-invariant' distributions $\Nvrnt({\mRat})$, $\Nvrnt({\aRat})$, and $\Nvrnt({\cRat})$.%\footnote{Szeidl's proof supplants simulation evidence of ergodicity that appeared in an earlier version of this paper.}

%A earlier draft of this paper~\cite{BufferStockTheoryQESubmit} conjectured that

\cite{harmenbergInvariant} cleverly substitutes a mathematical change of probability measure into \cite{szeidlInvariant}'s proof to show that under the {\GIC}, invariant \emph{permanent-income-weighted distributions} exist, which we follow Harmenberg by designating with a $\sim$ over the corresponding probability measure, e.g.\ the invariant Harmenberg-invariant distribution of $\mRat$ is $\tilde{\Nvrnt}(\mRat)$.\footnote{Harmenberg in a nutshell:  If $\pShk$ is described by density function $f_\pShk(\pShk)$, define the \emph{permanent-income-neutral measure} by $\tilde{f}_\pShk(\pShk) := \pShk f_\pShk(\pShk)$.  The condition for the existence of invariant permanent-income-weighted distributions is
  \begin{align*}
\log \PatPGro  & < \tilde{\Ex} \log \pShk
  \end{align*}
  where the $\sim$ over the expectations operator indicates that the expectation is being taken with respect to the permanent-income-neutral measure.  But by assumption $\tilde{\Ex} \log \pShk = 0$ so exponentiating both sides of this equation turns it into $\PatPGro < 1$ which is the {\GIC}.}  He is then able to prove a conjecture from an earlier draft of this paper (\cite{BufferStockTheoryQESubmit}) that under the {\GIC}, aggregate consumption grows at the same rate $\PGro$ as aggregate noncapital income in the long run.

\cite{harmenbergInvariant} also shows that his reformulation of the problem can reduce costs of calculation enormously.\footnote{The test of the Harmenberg method currently implemented in the Econ-ARK toolkit shows that, for the same computational resources, Harmenberg's method reduces the standard error of the simulated aggregate consumption series by a factor of 8; see the last part of \href{https://github.com/econ-ark/BufferStockTheory/blob/master/reproduce/computed.sh}{\texttt{reproduce/computed.sh}}.}
...
\subsection{Balanced Growth of Aggregate Income, Consumption, Wealth}\label{subsec:cGroEqPGroQ}
...
Consider an economy that has existed for long enough by date $t$ that by some measure we can consider it as Harmenberg-converged.  Under our assumption of lognormally distributed idiosyncratic permanent shocks, in such an economy a microeconomist with a population-representative dataset could calculate the growth rate of consumption for each individual household, and take the average to obtain:
\begin{align*}
...
\end{align*}
where $\pGro=\log \PGro$ and the last equality follows because the invariance of  $\tilde{\Nvrnt}({\cRat})$ means that $\Mean\left[ \log  {\cRat}_{t+n}\right] = \Mean\left[ \log c_{t}\right]$.  Thus in an economy that has settled into the Harmenberg-invariant distribution cross-sectional mean growth rates of noncapital income and consumption are the same.  The natural designation for the stable ratio of $\MLev$ to $\YLev$ is the `aggregate steady state' ratio.  Note that this is not numerically the same as the individual \href{https://econ-ark.github.io/BufferStockTheory/#pseudo-steady-state}{pseudo-steady-state} because the nonlinearities involved in simulation and aggregation will have consequences.\footnote{Still, the pseudo-steady-state can be calculated immediately from the policy function without any simulation, and therefore would likely serve as an excellent and low-cost starting point for the numerical simulation process.}
...
Total assets are
\begin{align*}
    \mathbf{A}_t & = \mathbb{M}[a_t\mathbf{p}_t] = \mathbb{M}[a_t]\mathbb{M}[\mathbf{p}_t]- \textup{cov}(a_t, \mathbf{p}_t).
\end{align*}

If by date $t$ the economy had achieved a Harmenberg-invariant state then it would follow that $\mathbb{M}[a_{t}]=\mathbb{M}[a_{t+1}]$ and $\mathbb{M}[\mathbf{p}_{t+1}]=\mathbb{M}[\mathbf{p}_t] \Gamma$ so that
\begin{align}
    %\mathbf{A}_{t+1} - \mathbf{A}_t & = (\PGro-1) \mathbf{A}_{t} + \PGro \textup{cov}(\aRat_{t},\mathbf{p}_{t}) - \textup{cov}(\aRat_{t+1},\mathbf{p}_{t+1})   \\
    \mathbf{A}_{t+1} + \textup{cov}(\aRat_{t+1},\mathbf{p}_{t+1})   & = \PGro ( \mathbf{A}_{t}  + \textup{cov}(\aRat_{t},\mathbf{p}_{t})) \label{eq:covProblem} .
\end{align}

Harmenberg's proof that an economy in the Harmenberg-invariant state achieves balanced growth implies that $\mathbf{A}_{t+1} = \PGro \mathbf{A}_{t}$.  In such an economy, \eqref{eq:covProblem} could be reduced to show that
\begin{align}
    \textup{cov}(\aRat_{t+1},\mathbf{p}_{t+1})   & = \PGro \textup{cov}(\aRat_{t},\mathbf{p}_{t}) \label{eq:covSoln} .
\end{align}

A corresponding argument shows that $\cov(\mRat,\pLev)$ also grows by $\PGro$.

\hypertarget{microfounding-macro-needs-ergodicity}{}
At first blush, this is a reassuring conclusion; one of the most persuasive arguments for the agenda of building microfoundations of macroeconomics is that microeconomic `big data' allow us to measure cross-sectional covariances like these with great precision, and we can use microeconomic natural experiments to disentangle questions that are hopelessly entangled in aggregate time-series data.  Knowing that such covariances ought to be a stable feature of a stably growing economy is therefore encouraging.

But the result also highlights an uncomfortable aspect of the approach: In the model as specified, permanent income does not, itself, have a limiting distribution; it becomes ever more dispersed as the economy with infinite-horizon consumers continues to grow indefinitely.
