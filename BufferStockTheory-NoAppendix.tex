...
\renewcommand{\forcedate}{October 9, 2021}\date{\forcedate}
...
Given the terminology developed earlier, the natural designation for the stable ratio of aggregate market resources to noncapital income is to call it the `aggregate steady state' ratio, $\StE{M}$.  Note that this is not numerically the same as the individual \href{https://econ-ark.github.io/BufferStockTheory/#pseudo-steady-state}{pseudo-steady-state} ratio $\mStE$ because the nonlinearities involved in simulation and aggregation will have consequences.\footnote{Still, the pseudo-steady-state can be calculated immediately from the policy function without any simulation, and therefore would likely serve as an excellent and low-cost starting point for the numerical simulation process.}
...
\end{align*}
\end{comment}
Consider again a Harmenberg-converged economy that has converged by date $t=0$.  Total assets are
\begin{align*}
...
As Harmenberg points out, a convenient feature of his measure is that $\Harm{\Mean}_{t+n}[\pLevBF_{t+n}]=\PGro^{n}$.  Using this, if by date $t=0$ the economy had achieved a Harmenberg-invariant state then it would follow that we could define $\bar{\aRat}=\Harm{\Mean}_{t}[a_{t}]$ and use the fact that thereafter assets grow at the constant rate $\PGro$ to obtain
\begin{align}
  \mathbf{A}_{t+1} & = \PGro \mathbf{A}_{t}\notag \\ 
  \bar{\aRat}\PGro +\HarmWide{\textup{cov}}(\aRat_{t+1},\pLevBF_{t+1})   & = \PGro \left( \bar{\aRat}  +  \HarmWide{\textup{cov}}(\aRat_{t},\pLevBF_{t})) \label{eq:covProblem} 
