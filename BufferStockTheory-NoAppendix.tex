...
\renewcommand{\forcedate}{October 7, 2021}\date{\forcedate}
...
Before we get to the formal argument below, the key insight can be understood already here, by considering the evolution of an economy that starts, at date $t$, with the entire population at $\mRat_{t}=\mStE$, but then evolves according to the model's assumed dynamics between $t$ and $t+1$.  Equation~\eqref{eq:balgrostable} will still hold, so for this first period, at least, the economy will exhibit balanced growth: the growth factor for aggregate $\mRatBF$ will match the growth factor for permanent income $\PGro$.  It is true that there will be people for whom $\bRat_{t+1}=\aRat_{t}\Rfree/(\PGro\pShk_{t+1})$ is boosted by a small draw of $\pShk_{t+1}$.  But their contribution to the aggregate variable is given by $\bRatBF_{t+1}=\bRat_{t+1}\pShk_{t+1}$, so their $\bRatBF_{t+1}$ is reweighted by an amount that exactly unwinds that boosting.
...
Cross-sectional and covariance relationships that economists can now observe in microeconomic datasets are increasingly central to macroeconomic research (see, e.g., \cite{violante_marginal_2021}'s recent Laffont lectures on relationship between the marginal propensity to consume and the distribution of assets).  But mapping from micro data to macro models can be more subtle than it seems.  Hence this final section's aim of carefully describing those relationships in the paper's simple model.

Formally, assume a continuum of \textit{ex ante} identical households on the unit interval, with constant total mass normalized to one and indexed by $i \in [0,1]$, all behaving according to the model specified above.  Szeidl~\citeyearpar{szeidlInvariant} provides a condition (intermediate between the {\GICNrm} and the {\GIC}) under which such a population will be characterized by invariant distributions of $\mRat$, $\cRat$, and $\aRat$;\footnote{\cite{szeidlInvariant}'s equation (9), in our notation, is:
  \begin{align*}
    \Ex \log \Rfree (1-\MPC) & < \Ex \log \PGro \pShk
    \\  \Ex \log \Rfree \PatR  &  < \Ex \log \PGro \pShk
\\ \log \PatPGro & < \Ex \log \pShk
  \end{align*}
  and under our default (though not required) assumption that $\log \pShk \sim \mathcal{N}(-\sigma^{2}_{\pShk}/2,\sigma^{2}_{\pShk})$ we can exponentiate both sides to obtain $\PatPGro < \exp(-\sigma^{2}_{\pShk}/2)$; the {\GICNrm} in this case, is $\PatPGro < \exp(-\sigma^{2})$, so if the {\GICNrm} holds then Szeidl's condition will hold.} designate these $\mathcal{F}^{\mRat}$, $\mathcal{F}^{\aRat}$, and $\mathcal{F}^{\cRat}$.%\footnote{Szeidl's proof supplants simulation evidence of ergodicity that appeared in an earlier version of this paper.}

\cite{harmenbergInvariant} cleverly substitutes a mathematical change of probability measure into \cite{szeidlInvariant}'s proof to obtain a condition under which invariant \emph{permanent-income-weighted distributions} exist.  In this case, aggregate consumption and wealth grow at the same rate as income --under a required condition that turns out to be the regular {\GIC}.  \cite{harmenbergInvariant} also shows that his reformulation of the problem can reduce costs of calculation exponentially -- a proposition that is confirmed by \href{}{this illustration} using the \href{}{HARK} toolkit.\footnote{Harmenberg in a nutshell:  If $\pShk$ is described by density function $f_\pShk(\pShk)$, define the \emph{permanent-income-neutral measure} by $\tilde{f}_\pShk(\pShk) := \pShk f_\pShk(\pShk)$.  The condition for the existence of invariant permanent-income-weighted distributions is
  \begin{align*}
\log \PatPGro  & < \tilde{\Ex} \log \pShk
  \end{align*}
  where the $\sim$ over the expectations operator indicates that the expectation is being taken with respect to the permanent-income-neutral measure.  But by assumption $\tilde{\Ex} \log \pShk = 0$ so exponentiating both sides of this equation turns it into $\PatPGro < 1$ which is the {\GIC}.}

\hypertarget{Consumption-and-Income-Growth-at-the-Household-Level}{}
\subsection{Identical Growth in Income, Consumption, and Wealth in Szeidl-Converged Economy}\label{subsec:cGroEqPGroIndQ}

Say that $\Mean\left[\bullet\right]$ yields the mean of its argument in the population, as distinct from the expectations operator $\Ex\left[\bullet\right]$ (which represents beliefs about the future).  In a Szeidl-converged economy, an economist with a microeconomic dataset could calculate the average consumption growth rate as:
\begin{align*}
...
\end{align*}
where $\pGro=\log \PGro$ and the last equality follows because the invariance of  $\mathcal{F}^{\cRat}$ means that $\Mean\left[ \log  {\cRat}_{t+n}\right] = \Mean\left[ \log c_{t}\right]$.  Thus the same {\GICNrm} that guaranteed the existence of an `individual target' value of $\mRat$ at the microeconomic level guarantees that in the Szeidl-invariant distribution the mean growth rates of income and consumption are the same (see~\cite{szeidlInvariant} for details) -- even though \emph{at the target} the growth rates are different (cf.\ Figure~\ref{fig:cGroTargetFig}).  A similar argument applies for growth in $\mLevBF$. 

\hypertarget{Growth-Rates-of-Aggregate-Income-and-Consumption}{}
\subsection{Balanced Growth of Aggregate Income, Consumption, Wealth, and Covariances}\label{subsec:cGroEqPGroQ}


Using boldface capitals for aggregates, the growth factor for aggregate income is:
\begin{align*}
...
Correspondingly, total assets are
\begin{align*}
    \mathbf{A}_t & = \mathbb{M}[a_t\mathbf{p}_t] = \mathbb{M}[a_t]\mathbb{M}[\mathbf{p}_t]- \textup{cov}(a_t, \mathbf{p}_t).
\end{align*}

If by date $t$ the distribution of $\aRat_{t}$ had reached its Szeidl-invariant distribution $\mathcal{F}^{\aRat}$ then it would follow that $\mathbb{M}[a_{t}]=\mathbb{M}[a_{t+1}]$ and $\mathbb{M}[\mathbf{p}_{t+1}]=\mathbb{M}[\mathbf{p}_t] \Gamma$ so that
\begin{align}
...
%Clearly if we knew that $\textup{cov}(\aRat_{t+1},\mathbf{p}_{t+1}) = \PGro \textup{cov}(\aRat_{t},\mathbf{p}_{t})$ then we would have $\mathbf{A}_{t+1} = \PGro \mathbf{A}_{t}$ and generic balanced growth in the aggregate.  

But the {\GICNrm} implies the {\GIC} and Harmenberg proves that the {\GIC} implies that $\mathbf{A}_{t+1} = \Gamma \mathbf{A}_{t}$, so if the {\GICNrm} holds then \eqref{eq:covProblem} can be reduced to show that
\begin{align}
\textup{cov}(\aRat_{t+1},\mathbf{p}_{t+1})   & = \PGro \textup{cov}(\aRat_{t},\mathbf{p}_{t}) \label{eq:covSoln} .
\end{align}

Unfortunately, the {\GICNrm} condition that we imposed that yielded all of these intuitive propositions at once is rather restrictive, and modeling experience indicates that it is not always possible to find plausible combinations of paramter values that satisfy that condition.
