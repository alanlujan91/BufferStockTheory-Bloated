...
\\ \hline
\cncl{\GICAbs}      & $\phantom{\Pat/\Rfree}$ & $\phantom{~<~}1        {~<~}$ & $        {\Pat/\PGro}$  & Constraint never binds for $\mRat \geq 1$ %& %$\{\Pat,\PGro\}=\{1,0.98\}$
...
\\ \GICAbs~            & $        {\Pat/\PGro}$ & $        {~<~}1\phantom{~<~}$ & $\phantom{\Pat/\Rfree}$  & Constraint binds in finite time for any $\mRat$ % &
...
\parbox{\TableWidth}{\footnotesize Conditions are applied from left to right; for example, the second row indicates conclusions in the case where \cncl{\GICAbs} and \RIC~both hold, while the third row indicates that when the \GICAbs~and the \RIC~both fail, the consumption function is degenerate; the next row indicates that whenever the \GICAbs~holds, the constraint will bind in finite time.}
...
\multicolumn{1}{|c|}{\GICAbs} & \multicolumn{1}{c|}{\GICNrm} \\ \hline
...
\multirow{4}{75mm}{\thead{By Jensen's inequality, stronger than \\the \GICAbs. \\  Ensures consumers will not \\ expect to accumulate $\mRat$ unboundedly.}} \\
& \\
& \\
& \\
